...
An example of this would a query again our MySQL checkouts table to return all rows ordered by the checkout_date column. The checkout_date column cannot be
...
to create the cache i one does not exist and then query from the cache to display the paged results.
...
origin data source (MySQL in this case). The "next" and "cursor" data will be passed along to function to query from the cache to return results.
